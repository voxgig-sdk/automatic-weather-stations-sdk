...
describe('FeatureCollectionDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new AutomaticWeatherStationsSDK({
  ...
  test('direct-list-feature_collection', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-feature_collection', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['AUTOMATIC_WEATHER_STATIONS_TEST_FEATURE_COLLECTION_ENTID']
