...
//     (\`new AutomaticWeatherStationsSDK(...)\` / \`AutomaticWeatherStationsSDK.test(...)\` become
//     \`AutomaticWeatherStationsSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'AutomaticWeatherStationsSDK'
...
const TEST_SEED = {"entity":{"collection":{"test01":{"id":"test01"}},"feature_collection":{"test01":{"id":"test01"}},"item":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new AutomaticWeatherStationsSDK(...)\` and any
// \`AutomaticWeatherStationsSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+AutomaticWeatherStationsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/AutomaticWeatherStationsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
