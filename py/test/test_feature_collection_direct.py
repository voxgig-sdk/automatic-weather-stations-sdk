...
class TestFeatureCollectionDirect:

    def test_should_direct_list_feature_collection(self):
        setup = _feature_collection_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-feature_collection", "live" if setup["live"] else "unit")
        if _skip:
...
def _feature_collection_direct_setup(mockres):
