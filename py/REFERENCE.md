# AutomaticWeatherStations Python SDK Reference

Complete API reference for the AutomaticWeatherStations Python SDK.
...
#### `AutomaticWeatherStationsSDK.test(testopts=None, sdkopts=None)`
...
#### `FeatureCollection(data=None)`

Create a new `FeatureCollectionEntity` instance. Pass `None` for no initial data.
...
```python
results, err = client.FeatureCollection().list({})
```
...
Create a new `FeatureCollectionEntity` instance with the same options.
