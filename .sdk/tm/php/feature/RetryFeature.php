...
// AutomaticWeatherStations SDK retry feature
...
class AutomaticWeatherStationsRetryFeature extends AutomaticWeatherStationsBaseFeature
{
    ...
    public function init(AutomaticWeatherStationsContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (AutomaticWeatherStationsContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(AutomaticWeatherStationsContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(AutomaticWeatherStationsContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
