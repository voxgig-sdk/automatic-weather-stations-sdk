...
// AutomaticWeatherStations SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = AutomaticWeatherStationsOmni::makeStructRunner(
    ...
    {
        return AutomaticWeatherStationsOmni::objmodel(AutomaticWeatherStationsOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return AutomaticWeatherStationsOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return AutomaticWeatherStationsOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(AutomaticWeatherStationsOmni::structfix($now, Struct::undef()))
                        !== json_encode(AutomaticWeatherStationsOmni::structfix($val, Struct::undef()))) {
