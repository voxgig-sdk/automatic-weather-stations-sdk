...
// AutomaticWeatherStations SDK feature test
//
...
class FtCtrl extends AutomaticWeatherStationsControl
{
...
class FtClient
{
    ...
    private AutomaticWeatherStationsUtility $utility;

    public function __construct(AutomaticWeatherStationsUtility $utility, array $options)
    {
    ...
    public function get_utility(): AutomaticWeatherStationsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (AutomaticWeatherStationsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public AutomaticWeatherStationsContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (AutomaticWeatherStationsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = AutomaticWeatherStationsConfig::make_feature($name);
    ...
    private static function buildUrl(AutomaticWeatherStationsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof AutomaticWeatherStationsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new AutomaticWeatherStationsResponse($fetched);
        }
    ...
    private function fail(AutomaticWeatherStationsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (AutomaticWeatherStationsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
