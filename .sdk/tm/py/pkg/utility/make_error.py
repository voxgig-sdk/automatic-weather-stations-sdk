...
from projectname_sdk.core.control import AutomaticWeatherStationsControl
from projectname_sdk.core.error import AutomaticWeatherStationsError


def make_error_util(ctx, err):
    if ctx is None:
        from projectname_sdk.core.context import AutomaticWeatherStationsContext
        ctx = AutomaticWeatherStationsContext({}, None)
    ...
    result = ctx.result
    if result is None:
        result = AutomaticWeatherStationsResult({})
    result.ok = False
    ...
    errmsg = ""
    if isinstance(err, AutomaticWeatherStationsError):
        errmsg = err.msg
    ...
    sdk_err = AutomaticWeatherStationsError("", msg, ctx)
