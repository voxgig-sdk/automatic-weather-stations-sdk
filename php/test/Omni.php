...
//    only - a typed AutomaticWeatherStationsContext there would make every ctx
...
final class AutomaticWeatherStationsOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new AutomaticWeatherStationsContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new AutomaticWeatherStationsSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new AutomaticWeatherStationsResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new AutomaticWeatherStationsError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new AutomaticWeatherStationsResponse($clean['response']);
    ...
    public static function err_from_map($m): ?AutomaticWeatherStationsError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new AutomaticWeatherStationsError(is_string($code) ? $code : '', $msg);
    }
