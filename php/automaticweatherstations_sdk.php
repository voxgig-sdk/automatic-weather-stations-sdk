...
class AutomaticWeatherStationsSDK
{
    public string $mode;
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = AutomaticWeatherStationsHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = AutomaticWeatherStationsHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!AutomaticWeatherStationsFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, AutomaticWeatherStationsFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return AutomaticWeatherStationsUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = AutomaticWeatherStationsHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = AutomaticWeatherStationsHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = AutomaticWeatherStationsHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new AutomaticWeatherStationsError($op . "_allow",
                "AutomaticWeatherStationsSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = AutomaticWeatherStationsHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new AutomaticWeatherStationsError("graphql_error",
                "AutomaticWeatherStationsSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    private $_feature_collection = null;

    // Canonical facade: $client->FeatureCollection()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->feature_collection()
    // resolves here too.
    public function FeatureCollection($data = null)
    {
        require_once __DIR__ . '/entity/feature_collection_entity.php';
        if ($data === null) {
            if ($this->_feature_collection === null) {
                $this->_feature_collection = new FeatureCollectionEntity($this, null);
            }
            return $this->_feature_collection;
        }
        return new FeatureCollectionEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new AutomaticWeatherStationsSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
