...
// AutomaticWeatherStations SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class AutomaticWeatherStationsTestFeature extends AutomaticWeatherStationsBaseFeature
{
    ...
    public function init(AutomaticWeatherStationsContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (AutomaticWeatherStationsContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = AutomaticWeatherStationsParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (AutomaticWeatherStationsContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(AutomaticWeatherStationsContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = AutomaticWeatherStationsParam::call($ctx, $k);
