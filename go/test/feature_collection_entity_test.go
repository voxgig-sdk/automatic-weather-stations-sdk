...
func TestFeatureCollectionEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.FeatureCollection(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.FeatureCollection(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		featureCollectionRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.feature_collection")))
...
func feature_collectionBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("AUTOMATIC_WEATHER_STATIONS_TEST_FEATURE_COLLECTION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["AUTOMATIC_WEATHER_STATIONS_TEST_FEATURE_COLLECTION_ENTID"])
