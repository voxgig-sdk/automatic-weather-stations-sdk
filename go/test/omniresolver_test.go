...
import (
	...
	omni "github.com/voxgig-sdk/automatic-weather-stations-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.AutomaticWeatherStationsError); is {
				out := map[string]any{
					"name":    "AutomaticWeatherStationsError",
...
func omniCtx(arg any, client *sdk.AutomaticWeatherStationsSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.AutomaticWeatherStationsSDK); is {
			client = live
