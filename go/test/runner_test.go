...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("AUTOMATICWEATHERSTATIONS_TEST_LIVE") == "TRUE" ||
		os.Getenv("AUTOMATICWEATHERSTATIONS_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("AUTOMATICWEATHERSTATIONS_TEST_EXPLAIN"); explain != "" {
