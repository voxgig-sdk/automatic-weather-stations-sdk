...
require("automatic-weather-stations_types")
...
local AutomaticWeatherStationsSDK = {}
AutomaticWeatherStationsSDK.__index = AutomaticWeatherStationsSDK
...
AutomaticWeatherStationsSDK._make_feature = _make_feature


function AutomaticWeatherStationsSDK.new(options)
  local self = setmetatable({}, AutomaticWeatherStationsSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function AutomaticWeatherStationsSDK:options_map()
...
function AutomaticWeatherStationsSDK:get_utility()
  return Utility.copy(self._utility)
end


function AutomaticWeatherStationsSDK:get_root_ctx()
  return self._rootctx
end


function AutomaticWeatherStationsSDK:prepare(fetchargs)
...
function AutomaticWeatherStationsSDK:direct(fetchargs)
...
function AutomaticWeatherStationsSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function AutomaticWeatherStationsSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "AutomaticWeatherStationsSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function AutomaticWeatherStationsSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function AutomaticWeatherStationsSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "AutomaticWeatherStationsSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function AutomaticWeatherStationsSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = AutomaticWeatherStationsSDK.new(sdkopts)
  sdk.mode = "test"
...
return AutomaticWeatherStationsSDK
