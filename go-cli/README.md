# automatic-weather-stations-cli

boru-driven command-line client **and** interactive REPL for the AutomaticWeatherStations
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/automatic-weather-stations-cli)
make build

# 2. See usage (words, entities, env vars)
./automatic-weather-stations-cli --help

# 3. Provide credentials once, via the environment
export AUTOMATIC_WEATHER_STATIONS_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./automatic-weather-stations-cli list collection
./automatic-weather-stations-cli list feature_collection

# 5. Override the API base URL for a single call
AUTOMATIC_WEATHER_STATIONS_BASE=https://api.example.com ./automatic-weather-stations-cli list collection

# 6. No arguments -> interactive REPL
./automatic-weather-stations-cli
automatic-weather-stations> list collection
automatic-weather-stations> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/automatic-weather-stations-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/automatic-weather-stations-cli list collection
   ```
...
Run with no arguments to open a REPL (prompt `automatic-weather-stations>`). Each line is
evaluated as its own boru expression:

```text
$ ./automatic-weather-stations-cli
automatic-weather-stations> list collection
automatic-weather-stations> /help
automatic-weather-stations> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/automatic-weather-stations-cli`. |
...
(`./automatic-weather-stations-cli <expr>`) and interactively (the REPL), and expressions compose the
