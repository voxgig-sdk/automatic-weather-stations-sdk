...
type Args struct {
	Entity string         `json:"entity" jsonschema:"collection | feature_collection | item"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.AutomaticWeatherStationsSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "automatic-weather-stations_list",
		Description: "List records from AutomaticWeatherStations. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "automatic-weather-stations_load",
		Description: "Load a single record from AutomaticWeatherStations. " +
...
func runOp(client *sdk.AutomaticWeatherStationsSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.AutomaticWeatherStationsSDK, name string) (sdk.AutomaticWeatherStationsEntity, error) {
	switch strings.ToLower(name) {
	case "collection":
		return client.Collection(nil), nil
	case "feature_collection":
		return client.FeatureCollection(nil), nil
	case "item":
