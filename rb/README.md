# AutomaticWeatherStations Ruby SDK



The Ruby SDK for the AutomaticWeatherStations API — an entity-oriented client using idiomatic Ruby conventions.
...
```ruby
client = AutomaticWeatherStationsSDK.test(testopts, sdkopts)
```
...
raise a `AutomaticWeatherStationsError` (a `StandardError` subclass), so wrap
...
The main module (`AutomaticWeatherStations_sdk`) exports the SDK class
