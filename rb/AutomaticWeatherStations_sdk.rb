...
require_relative 'AutomaticWeatherStations_types'
...
class AutomaticWeatherStationsSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = AutomaticWeatherStationsHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = AutomaticWeatherStationsHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, AutomaticWeatherStationsFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    AutomaticWeatherStationsUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = AutomaticWeatherStationsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = AutomaticWeatherStationsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = AutomaticWeatherStationsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => AutomaticWeatherStationsError.new(
        "#{op}_allow",
  ...
  def raw_request(fetchargs = {})
    ...
    rescue AutomaticWeatherStationsError => err
    ...
    if fetched.is_a?(Hash)
      status = AutomaticWeatherStationsHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = AutomaticWeatherStationsError.new(
        "graphql_error", "AutomaticWeatherStationsSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = AutomaticWeatherStationsSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
