...
class FeatureCollectionDirectTest < Minitest::Test
  def test_direct_list_feature_collection
    setup = feature_collection_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-feature_collection", setup[:live] ? "live" : "unit")
    if _should_skip
...
def feature_collection_direct_setup(mockres)
