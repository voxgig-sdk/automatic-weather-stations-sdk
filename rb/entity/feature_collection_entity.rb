...
class FeatureCollectionEntity
  ...
  def make
    opts = @_entopts.dup
    FeatureCollectionEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = AutomaticWeatherStationsHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [FeatureCollection, Hash] the current FeatureCollection data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = AutomaticWeatherStationsHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of FeatureCollection fields)
  def match_get
  ...
  #
  # @param reqmatch [FeatureCollectionListMatch, Hash, nil] match filter (any subset of
  #   FeatureCollection fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<FeatureCollection>, Array] the matching FeatureCollection items; raises AutomaticWeatherStationsError on failure
