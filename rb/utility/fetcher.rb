# AutomaticWeatherStations SDK utility: fetcher
...
module AutomaticWeatherStationsUtilities
  ...
  DefaultHttpFetch = ->(fullurl, fetchdef) {
    ...
    begin
      ...
      request['User-Agent'] = 'Mozilla/5.0 (compatible; AutomaticWeatherStationsSDK/1.0)' unless has_ua
      request.body = body_str if body_str.is_a?(String)

      http = AutomaticWeatherStationsUtilities.http_checkout(key, uri, proxy)
      begin
        # Net::HTTP retries idempotent requests; an EOF can follow a committed POST.
        resp = http.request(request)
      rescue StandardError
        AutomaticWeatherStationsUtilities.http_discard(http)
        raise
      end
      AutomaticWeatherStationsUtilities.http_checkin(key, http)
